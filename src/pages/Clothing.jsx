...
export default function Clothing() {
  ...
  const [clothing, setData] = useState([]);
  ...
  const fetchClothing = async () => {
    setLoading(true);
    const qs = await getDocs(collection(db, 'clothing'));
    const res = [];
    let resSum = 0;
    qs.forEach((doc) => {
      let item = doc.data();
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedItem) {
      ...
      (async () => {   
        try {
          await addDoc(collection(db, 'clothing'), newItem);
          handleClose();
          fetchClothing();
        } catch (e) {
  ...
  return (
    <div className='page clothing'>

      <div className="clothing_header">
        <div className="left">
          <h3 className="page_title">Clothing</h3>
